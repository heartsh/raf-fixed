...
char MultiSequence::GetAnnotationChar(const std::vector<char> &column) const
{
    ...
    char annot[] = ":::::::::...........";
    char *groups[20] = { "STA", "NEQK" "NHQK", "NDEQ", "QHRK",
...
{  
    ...
    outfile << "CLUSTALW (CONTRAlign) multiple sequence alignment" << std::endl;
...
std::pair<std::vector<int>, std::vector<int> > MultiSequence::GetAlignedTo() const
{
    ...
    std::vector<int> aligned_to_x(sequences[0]->GetCompressedLength() + 1, UNKNOWN);
    std::vector<int> aligned_to_y(sequences[1]->GetCompressedLength() + 1, UNKNOWN);
